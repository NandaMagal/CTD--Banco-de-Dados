# SCHEMA NETFLIX NANDA

CREATE SCHEMA IF NOT EXISTS `NETFLIX_NANDA` DEFAULT CHARACTER SET utf8mb4 ;
USE `NETFLIX_NANDA` ;


#TABELA ATORES

CREATE TABLE IF NOT EXISTS `NETFLIX_NANDA`.`ATORES` (
  `Id_Atores` int NOT NULL AUTO_INCREMENT,
  `Nome_Ator` varchar(45) DEFAULT NULL,
  `Sobren_Ator` varchar(45) DEFAULT NULL,
  `Data_Nasc_Ator` date NULL,
...
CREATE TABLE IF NOT EXISTS `SERIES`(
  ...
  `IdGenero_Series` int NOT NULL ,
  `Nome_Serie` varchar(45) DEFAULT NULL,
  `Descricao` text NULL,
  `Classificacao` Int,
  PRIMARY KEY (`IdSerie`),
  KEY `fk_Id_Genero_Series` (`Id_Genero`) ,
  CONSTRAINT `fk_Id_Genero_Series` FOREIGN KEY (`Id_Genero`) REFERENCES `NETFLIX_NANDA`.`GENEROS`
) ;
